...
import { checkCanCreate, mint } from "../utils/contracts";
...
const CreateNFT = () => {
  ...
  const handleSubmit = async (formData: NFTModel) => {
    ...
    const metaUploaded = await ipfs.add(
      Buffer.from(
        JSON.stringify({
          name: nftData.name,
      ...
      <div>
        <div className="px-4  ">
          ...
          {account ? (
            <div className="flex flex-col justify-between lg:flex-row lg:items-start">
              <div className="flex-2 lg:pr-12 ">
                <NFTForm
                  ...
                  onSubmit={(formData: NFTModel) => handleSubmit(formData)}
                  onChangeData={(formData: NFTModel) => handleChange(formData)}
                />
              </div>
              <div className="flex-1 mt-8">
                <p className="mb-3 font-bold text-left text-br-dark-blue">
                  Preview
                </p>
            ...
            <>
              <p className="mt-5">
                Please connect Wallet to create a collectible
              </p>
            </>
          )}
