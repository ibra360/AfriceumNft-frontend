...
const Search = (props: any) => {
  ...
  console.log("search items,", items, users);

  return (
    <div
      className="px-4 pt-20 lg:px-0 lg:px-56"
      style={{ backgroundColor: "white" }}
    >
      <h2 className="text-2xl font-bold text-left text-gray-800">
        Search results for{" "}
        <span className="text-black">{location?.state?.query}</span>{" "}
      </h2>

      <div className="flex items-center justify-between mt-20">
        <div className="flex justify-start space-x-8 ">
          <button
            onClick={() => setActive("collectibles")}
            className={
              " hover:text-gray-800 font-bold " +
              (active === "collectibles" ? "text-black" : "text-gray-600")
            }
          >
            Collectibles{" "}
          </button>
          <button
            ...
            className={
              "hover:text-gray-800 font-bold " +
              (active === "users" ? "text-black" : "text-gray-600")
            }
          >
            Users
          </button>
        ...
        <div className="flex items-center">
          ...
          <div className="w-full mb-4 text-black border-2 border-gray-800 rounded-full bg-br-gray">
            <select
              className={"px-2 py-2 pr-4 w-full border-none rounded-3xl"}
      ...
      {active === "users" && (
        <div
          className={`all-nfts-wrapper grid transition ease-in gap-7 w-full grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 2xl:grid-cols-6`}
        >
      ...
      {active === "collectibles" && (
        <div
          className={`all-nfts-wrapper grid transition ease-in gap-7 w-full grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5`}
        >
      ...
      {items.length === 0 && users.length === 0 && (
        <div
          className={`all-nfts-wrapper grid transition ease-in gap-7 w-full grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5`}
        >
          <p className="h-[250px]">No result</p>
        </div>
      )}
